...
export function Producao() {
  ...
  const loadProjetos = async () => {
    try {
      const { data, error } = await supabase
        .from("projects")
        .select(
          "id, titulo_ambiente, status, created_at, clients(nome, telefone)",
        )
  ...
  const notifyStatusChange = async (projeto: Projeto, novoStatus: Status) => {
    const telefone = projeto.clients?.telefone;
    if (!telefone || !N8N_WEBHOOK_URL) return;
    try {
      await fetch(N8N_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          telefone,
          cliente_nome: projeto.clients?.nome ?? "",
          ambiente: projeto.titulo_ambiente,
          status_anterior:
            COLUNAS.find((c) => c.status === projeto.status)?.label ??
            projeto.status,
          status_novo:
            COLUNAS.find((c) => c.status === novoStatus)?.label ?? novoStatus,
          projeto_id: projeto.id,
        }),
      });
    } catch (err) {
      console.warn("Aviso: falha ao notificar n8n:", err);
    }
  };
  ...
  const moverStatus = async (
    ...
    try {
      ...
      // Dispara notificacao WhatsApp em background (nao bloqueia UI)
      notifyStatusChange(projeto, novoStatus);
    } catch (err) {
          ...
          {COLUNAS.map((coluna) => {
            ...
            return (
              <div key={coluna.status} className="w-72 flex-shrink-0">
                {/* Header da coluna */}
                <div
                  className={`flex items-center justify-between px-4 py-3 rounded-t-xl border-2 border-b-0 ${coluna.bg}`}
                >
                  <span className={`font-bold text-sm ${coluna.color}`}>
                    {coluna.label}
                  </span>
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full bg-white/70 ${coluna.color}`}
                  >
                    {cards.length}
                  </span>
                </div>
                ...
                {/* Cards */}
                <div
                  className={`min-h-[400px] rounded-b-xl border-2 border-t-0 ${coluna.bg} p-2 space-y-2`}
                >
                  {cards.length === 0 && (
                      ...
                      >
                        ...
                        <div className="flex gap-1 pt-1">
                          ...
                          <button
                            ...
                            disabled={
                              idx === STATUS_ORDER.length - 1 || isUpdating
                            }
                            className="flex-1 flex items-center justify-center gap-1 h-7 text-xs font-medium rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition"
                          >
                            {isUpdating ? (
